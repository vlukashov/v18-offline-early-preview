...
export class PersonFormViewElement extends LitElement {
  ...
  private async save() {
    try {
      ...
      this.clearForm();
      const message = result.isDeferred ?
        'Person details stored offline and will be synced with the server after coming online.' :
        'Person details stored.';
      showNotification(message, {position: 'bottom-start'});
    } catch (error) {
